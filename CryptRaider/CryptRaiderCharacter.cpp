	...
	: IsHoldingItem(false)
{
	...
	GrabbedItemSpot = CreateDefaultSubobject<USceneComponent>("Grabbed Item Spot");
	GrabbedItemSpot->SetupAttachment(RootComponent);
}
...
{	
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		// Open menu
		EnhancedInputComponent->BindAction(OpenMenuAction, ETriggerEvent::Triggered, this, &ACryptRaiderCharacter::OpenMenu);
	}
	else
	{
...
UGrabber* ACryptRaiderCharacter::GetGrabberComponent()
{
...
{
	UE_LOG(LogTemp, Warning, TEXT("Pressed Grab action"));
	//AActor* GrabbedActor = nullptr;
	if (IsHoldingItem)
	{
		UE_LOG(LogTemp, Warning, TEXT("Is holding Item: trying to release"));

		GrabberComponent->ReleaseItem();
		IsHoldingItem = false;
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("Not holding item: trying to grab something"));
		bool HasGrabbedItem = GrabberComponent->TryToGrab();
		if (HasGrabbedItem)
		{
			IsHoldingItem = true;
		}
